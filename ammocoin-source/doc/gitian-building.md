...
*Setup instructions for a gitian build of AMMOcoin using a Debian VM or physical system.*
...
- [Building AMMOcoin](#building-ammocoin)
...
Clone the git repositories for ammocoin and gitian and then checkout the ammocoin version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/ammocoin-crypto/ammocoin.git
cd ammocoin
...
Follow the instructions in [doc/release-process.md](release-process.md) in the ammocoin repository
...
To build AMMOcoin (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the ammocoin repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/ammocoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/ammocoin-crypto/ammocoin
...
For example:
```bash
URL=https://github.com/crowning-/ammocoin.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit ammocoin=${COMMIT} --url ammocoin=${URL} ../ammocoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit ammocoin=${COMMIT} --url ammocoin=${URL} ../ammocoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit ammocoin=${COMMIT} --url ammocoin=${URL} ../ammocoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/ammocoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/ammocoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/ammocoin-build.assert
```
...
[ammocoin/gitian.sigs](https://github.com/ammocoin-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
