...
// The zAMMO block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zAMMO is a hash of the serial
...
{
    //Create an output returning the zAMMO that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZAMMOOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zAMMO output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zAMMO", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZAMMOOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zAMMO output", __func__);
...
{
    CzAMMOTracker* zammoTracker = pwallet->zammoTracker.get();
    CMintMeta meta;
    if (!zammoTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zammoTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a AMMO stake is the outpoint
