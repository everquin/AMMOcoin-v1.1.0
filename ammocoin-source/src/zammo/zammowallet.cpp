...
CzAMMOWallet::CzAMMOWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zammo seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZAMMOSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZAMMOSeed_deprecated()) {
                    LogPrintf("%s: Updated zAMMO seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zammo seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zammo wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzAMMOWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZAMMOCount(nCountLastUsed);
    else if (!walletdb.ReadZAMMOCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzAMMOWallet::Lock()
{
    seedMaster = 0;
}

void CzAMMOWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzAMMOWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZAMMO(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzAMMOWallet::LoadMintPoolFromDB()
{
...
void CzAMMOWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzAMMOWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzAMMOWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zammoTracker->HasPubcoinHash(pMint.first)) {
...
bool CzAMMOWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZAMMO(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zammoTracker which also adds to database
    pwalletMain->zammoTracker->Add(dMint, true);
...
void CzAMMOWallet::SeedToZAMMO(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzAMMOWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzAMMOWallet::GenerateDeterministicZAMMO(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzAMMOWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzAMMOWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzAMMOWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
