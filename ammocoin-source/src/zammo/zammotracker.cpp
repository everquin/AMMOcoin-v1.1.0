...
CzAMMOTracker::CzAMMOTracker(std::string strWalletFile)
{
...
CzAMMOTracker::~CzAMMOTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzAMMOTracker::Init()
{
...
bool CzAMMOTracker::Archive(CMintMeta& meta)
{
...
bool CzAMMOTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzAMMOTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzAMMOTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzAMMOTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzAMMOTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzAMMOTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzAMMOTracker::GetSerialHashes()
{
...
CAmount CzAMMOTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzAMMOTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzAMMOTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzAMMOTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzAMMOTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzAMMOTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzAMMOTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzAMMOTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzAMMOTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzAMMOTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzAMMOTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzAMMOWallet* zAMMOWallet)
{
    bool iszAMMOWalletInitialized = (NULL != zAMMOWallet);
    CMintMeta meta;
    ...
    if (! iszAMMOWalletInitialized)
        zAMMOWallet = new CzAMMOWallet(strWalletFile);
    meta.isSeedCorrect = zAMMOWallet->CheckSeed(dMint);
    if (! iszAMMOWalletInitialized)
        delete zAMMOWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzAMMOTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzAMMOTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzAMMOTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzAMMOTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzAMMOTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzAMMOTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzAMMOWallet* zAMMOWallet = new CzAMMOWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zAMMOWallet);
        }
        delete zAMMOWallet;
        LogPrint("zero", "%s: added %d dzammo from DB\n", __func__, listDeterministicDB.size());
    }
