...
#include "ui_zammocontroldialog.h"
...
void ZPivControlDialog::updateList()
{
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        {
            LOCK(pwalletMain->zammoTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zammoTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zAMMO.";
            else if (!mint.isSeedCorrect)
                strReason = "The zAMMO seed used to mint this zAMMO is not the same as currently hold in the wallet";
            else
