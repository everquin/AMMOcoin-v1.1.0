...
public:
    ...
    bool WriteZAMMOSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZAMMOSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZAMMOSeed_deprecated(uint256& seed);
    bool EraseZAMMOSeed();
    bool EraseZAMMOSeed_deprecated();

    bool WriteZAMMOCount(const uint32_t& nCount);
    bool ReadZAMMOCount(uint32_t& nCount);
