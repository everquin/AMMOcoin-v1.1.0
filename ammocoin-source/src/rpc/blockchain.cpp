...
UniValue blockToJSON(const CBlock& block, const CBlockIndex* blockindex, bool txDetails = false)
{
    ...
    UniValue zammoObj(UniValue::VOBJ);
    for (auto denom : libzerocoin::zerocoinDenomList) {
        zammoObj.push_back(Pair(to_string(denom), ValueFromAmount(blockindex->mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    zammoObj.push_back(Pair("total", ValueFromAmount(blockindex->GetZerocoinSupply())));
    result.push_back(Pair("zAMMOsupply", zammoObj));
...
UniValue getchecksumblock(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw runtime_error(
            ...
            "  \"zAMMOsupply\" :\n"
            "  {\n"
            "     \"1\" : n,            (numeric) supply of 1 zAMMO denomination\n"
            "     \"5\" : n,            (numeric) supply of 5 zAMMO denomination\n"
            "     \"10\" : n,           (numeric) supply of 10 zAMMO denomination\n"
            "     \"50\" : n,           (numeric) supply of 50 zAMMO denomination\n"
            "     \"100\" : n,          (numeric) supply of 100 zAMMO denomination\n"
            "     \"500\" : n,          (numeric) supply of 500 zAMMO denomination\n"
            "     \"1000\" : n,         (numeric) supply of 1000 zAMMO denomination\n"
            "     \"5000\" : n,         (numeric) supply of 5000 zAMMO denomination\n"
            "     \"total\" : n,        (numeric) The total supply of all zAMMO denominations\n"
            "  }\n"
            "}\n"
...
UniValue getrawmempool(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "    \"fee\" : n,              (numeric) transaction fee in ammocoin\n"
...
UniValue getaccumulatorwitness(const UniValue& params, bool fHelp)
{
    ...
    if (!GenerateAccumulatorWitness(pubCoin, accumulator, witness, nMintsAdded, strFailReason)) {
        receipt.SetStatus(_(strFailReason.c_str()), ZAMMO_FAILED_ACCUMULATOR_INITIALIZATION);
