...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "ammocoin" is a composite category enabling all AMMOcoin-related debug output
            if (ptrCategory->count(string("ammocoin"))) {
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "ammocoin.conf"));
    ...
    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty ammocoin.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override ammocoin.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "ammocoind.pid"));
